...
2. Now, an SDK has to be installed which provides DISCO with some important commands. In order to install this SDK, some packages might have to be upgraded:
    ...
    *NOTE*: Depending on your system, you might have to install other packages such as pynacl, cryptography, python-openstackclient/python-novaclient or others. Potential error messages during the installation process will make you aware of those. In that case, just install them like urllib3[secure] under 5.
